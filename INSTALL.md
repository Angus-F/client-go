...
This will record a dependency on `github.com/Angus-F/client-go` in your go module.
You can now import and use the `github.com/Angus-F/client-go` APIs in your project.
The next time you `go build`, `go test`, or `go run` your project,
`github.com/Angus-F/client-go` and its dependencies will be downloaded (if needed),
...
If you want to use a particular version of the `github.com/Angus-F/client-go` library,
...
- If you are using Kubernetes versions >= `v1.17.0`, use a corresponding `v0.x.y` tag.
  For example, `github.com/Angus-F/client-go@v0.20.4` corresponds to Kubernetes `v1.20.4`:
...
- If you are using Kubernetes versions < `v1.17.0`, use a corresponding `kubernetes-1.x.y` tag.
  For example, `github.com/Angus-F/client-go@kubernetes-1.16.3` corresponds to Kubernetes `v1.16.3`:

```sh
go get github.com/Angus-F/client-go@kubernetes-1.16.3
```
...
`module github.com/Angus-F/client-go@latest found (v1.5.2), but does not contain package github.com/Angus-F/client-go/...`,
you are likely using a go version prior to 1.16 and must explicitly specify the github.com/Angus-F/client-go version you want.
For example:
```sh
go get github.com/Angus-F/client-go@v0.20.4
```
...
something in your build is likely requiring an old version of `github.com/Angus-F/client-go` like `v11.0.0+incompatible`.
...
```sh
go mod graph | grep " github.com/Angus-F/client-go@"
```
...
```sh
go mod edit -replace=github.com/Angus-F/client-go=github.com/Angus-F/client-go@v0.20.4
go get github.com/Angus-F/client-go@v0.20.4
```
